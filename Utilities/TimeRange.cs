...
public record struct TimeRange(DateTimeOffset From, DateTimeOffset To)
{
    ...
    public string ToString(string format = "g", IFormatProvider? formatProvider = null)
    {
        formatProvider ??= CultureInfo.InvariantCulture;
        return $"{From.ToString(format, formatProvider)} to {To.ToString(format, formatProvider)}";
    }
}
